...
Cuando la petición corresponda a un \textit{IRS} admitido, el sistema debe identificar sus términos económicos y contractuales. Como mínimo, la solicitud debe permitir determinar el tipo de interés, la fecha de valoración, la fecha de inicio, la fecha de vencimiento, la divisa, el nocional y la dirección de la operación, expresada mediante la posición receptora o pagadora del tipo fijo. Estos campos son necesarios para representar de forma inequívoca la operación solicitada.
...
Cuando alguno de estos atributos no aparezca expresamente en la petición, el sistema podrá completarlo únicamente si existe una convención de mercado previamente definida para la divisa, el índice y el tipo de producto correspondientes. La \textit{RFQ} debe permitir distinguir los valores extraídos directamente del texto de aquellos obtenidos mediante estas convenciones. Si falta alguno de los campos obligatorios o no existe una regla aplicable, la solicitud debe considerarse incompleta.
...
Antes de aceptar la \textit{RFQ}, el sistema debe comprobar que los valores obtenidos sean completos y coherentes. Estas comprobaciones deben detectar, entre otros casos, fechas imposibles o desordenadas, nocionales o tipos no admisibles, incompatibilidades entre la divisa y el índice, contradicciones en la dirección de la operación y errores en la estructura de las patas. Si se detecta un error susceptible de corrección, el proceso podrá repetir la interpretación, con un límite máximo de cinco intentos. Si no se obtiene una \textit{RFQ} válida dentro de ese límite, el sistema debe finalizar con un resultado de error en lugar de producir una operación no fiable.
...
\hline
RF-03 & El sistema debe extraer el tipo de interés, las fechas de valoración, inicio y vencimiento, la divisa, el nocional y la dirección de la operación. \\
\hline
RF-04 & El sistema debe representar de forma diferenciada las patas fija y flotante y sus correspondientes atributos. \\
\hline
RF-05 & El sistema debe completar los campos no explícitos únicamente mediante convenciones de mercado previamente definidas. \\
\hline
RF-06 & El sistema debe distinguir entre los valores extraídos de la solicitud y los derivados mediante convenciones. \\
\hline
...
\hline
RF-09 & El sistema debe comprobar la integridad, la coherencia temporal y la compatibilidad financiera de la operación. \\
\hline
RF-10 & El sistema debe permitir corregir una salida inválida mediante un proceso limitado a un máximo de cinco intentos. \\
\hline
...
Otro requisito fundamental es la trazabilidad. Para cada ejecución debe ser posible identificar la petición recibida, la configuración utilizada, el resultado de la validación y la \textit{RFQ} producida, cuando corresponda. También debe registrarse qué valores proceden directamente del texto y cuáles han sido completados mediante convenciones de mercado. Esta información permite analizar los errores, justificar el origen de los datos y comprobar que las conclusiones experimentales se corresponden con una configuración concreta del sistema.
...
\hline
RNF-04 & El sistema debe identificar la procedencia de los valores, distinguiendo los extraídos del texto de los obtenidos mediante convenciones. \\
\hline
RNF-05 & Los experimentos deben ser reproducibles mediante la conservación de modelos, parámetros, instrucciones, dependencias y casos de prueba. \\
\hline
RNF-06 & Las ejecuciones realizadas con configuraciones o referencias diferentes deben permanecer separadas e identificadas. \\
\hline
RNF-07 & El sistema debe permitir medir la corrección, la latencia, el consumo de recursos y el coste de cada ejecución. \\
\hline
RNF-08 & La definición financiera, las reglas de validación y la configuración de los modelos deben poder modificarse de manera independiente. \\
\hline
RNF-09 & El sistema debe permitir la incorporación futura de nuevos productos financieros y proveedores de modelos sin sustituir su estructura fundamental. \\
\hline
RNF-10 & Las credenciales y los secretos de acceso no deben almacenarse en el código fuente ni incorporarse al repositorio. \\
\hline
RNF-11 & Los errores y rechazos deben expresarse de forma comprensible y permitir distinguir una petición inválida de un fallo técnico. \\
\hline
\end{tabular}
\floatfoot{Fuente: elaboración propia.}
\end{table}
...
Dentro de este alcance, la petición debe permitir identificar siete términos obligatorios: el tipo de interés fijo, la fecha de valoración, la fecha de inicio, la fecha de vencimiento, la divisa, el nocional y la dirección de la operación. Esta última determina si el solicitante paga o recibe el tipo fijo. La ausencia de cualquiera de estos elementos impide definir la operación de manera inequívoca y, por tanto, debe dar lugar a una petición incompleta.
...
El alcance contempla vencimientos superiores a un año. Las operaciones con vencimiento igual o inferior a un año quedan excluidas porque pueden seguir convenciones propias del mercado monetario, diferentes de las empleadas en los swaps con pagos periódicos. Esta exclusión evita aplicar automáticamente convenciones de un \textit{IRS} estándar a instrumentos cuya estructura temporal y forma de liquidación pueden ser distintas.

La representación de la operación debe diferenciar explícitamente la pata fija y la pata flotante. Cada pata incluye su frecuencia, su convención de recuento de días y sus fechas de pago. La pata flotante incorpora además el índice de referencia, su tenor, el posible diferencial y la curva utilizada para proyectar los tipos variables. El alcance admite calendarios con periodos no regulares, siempre que las fechas necesarias puedan determinarse de forma coherente a partir de la petición y de las reglas aplicables.
...
\hline
Vencimiento & Superior a un año. & Igual o inferior a un año. \\
\hline
